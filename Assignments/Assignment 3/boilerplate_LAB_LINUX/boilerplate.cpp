...
void createQuadratic(vector<float>& vertices, float x1, float x2, float x3, float y1, float y2, float y3);
...
bool InitializeGeometry(MyGeometry *geometry, const vector<float>& vertices)
{
...
void RenderScene(MyGeometry *geometry, MyShader *shader, GLuint renderMode = GL_LINE_STRIP)
{
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	if (key == GLFW_KEY_1)
	{
		...
		vertices.clear();
		createQuadratic(vertices, 1, 2, 0, 1, -1, -1);
		createQuadratic(vertices, 0, -2, -1, -1, -1, 1);
		createQuadratic(vertices, -1, 0, 1, 1, 1, 1);
		createQuadratic(vertices, 1.2, 2.5, 1.3, 0.5, 1.0, -0.4);
		drawingMode = GL_TRIANGLES;
		InitializeGeometry(&geometry, vertices);
	}
}
...
int main(int argc, char *argv[])
{
	...
	if (!InitializeGeometry(&geometry, vertices))
	...
	MyGlyph glyph = extractor.ExtractGlyph('a');
	cout << "Distance to next glyph" << glyph.advance << endl;
	cout << "Number of contours in a " << glyph.contours.size() << endl;

	MyContour contour = glyph.contours[0];
	cout << "Number of bezier curves in contour [0] " << contour.size() << endl;

	MySegment segment = contour[0];
	cout << "Degree of segment " << segment.degree << endl;
	cout << "Number of control points " << segment.degree + 1 << endl;
	cout << "Control poiunts coordinates: " << endl;
	for (unsigned i = 0; i < segment.degree; i++)
	{
		cout << "(" << segment.x[i] << ", " << segment.y[i] << ")" << endl;
	}
...
void createQuadratic(vector<float>& vertices, float x1, float x2, float x3, float y1, float y2, float y3)
{
	...
	float u = 0.;
	float sx = (1-u)*(1-u)*x1 + 2*u*(1-u)*x2 + u*u*x3;
	float sy = (1-u)*(1-u)*y1 + 2*u*(1-u)*y2 + u*u*y3;
	cout << sx  <<" SFDF" << sy<< endl;
	...
	{
		vertices.push_back((pow(1-i,2)*x1 + 2*i*(1-i)*x2 + pow(i,2)*x3));
		vertices.push_back((pow(1-i,2)*y1 + 2*i*(1-i)*y2 + pow(i,2)*y3));
	}
