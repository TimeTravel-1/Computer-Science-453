...
bool InitializeGeometry(MyGeometry *geometry)
{
	...
	for(float i=0;i<24;i++){ // U-points
 		for(float j=0;j<24;j++){ // V-points
 			...
 			vertices.push_back(sin(u)*sin(v));
 			vertices.push_back(cos(u));
 			vertices.push_back(sin(u)*cos(v));

 			vertices.push_back(sin(un)*sin(v));
 			vertices.push_back(cos(un));
 			vertices.push_back(sin(un)*cos(v));

 			vertices.push_back(sin(u)*sin(vn));
 			vertices.push_back(cos(u));
 			vertices.push_back(sin(u)*cos(vn));

 			vertices.push_back(sin(un)*sin(vn));
 			vertices.push_back(cos(un));
 			vertices.push_back(sin(un)*cos(vn));
	...
	const GLfloat normals[][3] = {
		...
		{.577, .577, -.577},
		{.577, -.577, -.577},
		//Normal to sphere would be from cener to point
	};

	const float textureCoords[16] = {
		0.f, 0.f, 
		0, 1.f, 
		1.f, 1.f, 
		1.f, 0.f, 
		...
		//mapping texture to face
	};


	geometry->elementCount = vertices.size()/3.f;
...
static void cursorPosCallback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (lbutton_down)
	{
		...
		else if (abs(xpos-xv)>abs(ypos-yv))
		{
			if (xpos > xv)
			{
				phi -= 0.01;
...
void updateCamera(vec3& cameraLoc, vec3& cameraDir, double& phi, double& theta, float& cameraRadius, GLFWwindow* window, int width, int height)
{
	...
	if(insideWindow)
	{
		...
		{	phi -= 0.01;
			cameraLoc = vec3(cameraRadius*sin(phi)*sin(theta), cameraRadius*cos(theta), cameraRadius*cos(phi)*sin(theta));
		}
		...
		{
			cameraRadius -=movespeed;
			if ((cameraRadius <= 2.0f))
			{
				cameraRadius = 2.0f;
			}
			if ((cameraRadius >= 20.0f))
			{
				cameraRadius = 20.0f;
			}
		}
...
////////////////////////////////////////////////////////////
    ...
    angle = angle + .01f;
		...
		updateCamera(cameraLoc, cameraDir, phi, theta, cameraRadius, window, width, height);

		mat4 model = translate(I, location) * rotate(I, angle, axis) * rotate(I, angle, vec3(1,0,0)) * scale(I, vec3(size, 1, 1));
		mat4 view = lookAt(cameraLoc, vec3(0,0,0)-cameraLoc, cameraUp);
		mat4 proj = perspective(fov, aspectRatio, zNear, zFar);
		glUniformMatrix4fv(modelUniform, 1, false, value_ptr(I));
		...
		glUseProgram(shader.program);
		mat4 model2 = translate(I, vec3(4.08746579676,0,0)) * rotate(I, angle, axis) * rotate(I, angle, vec3(1,0,0)) * scale(I, vec3(0.4906952587, 0.4906952587, 0.4906952587));
		glUniformMatrix4fv(modelUniform, 1, false, value_ptr(model2));
		RenderScene(&geometry, &shader, &texture);

		glUseProgram(shader.program);
		mat4 model3 = translate(I, vec3(4.08746579676,0,0)) * rotate(I, angle, axis) * rotate(I, angle, vec3(1,0,0)) * scale(I, vec3(0.38420023835, 0.38420023835, 0.38420023835));
		glUniformMatrix4fv(modelUniform, 1, false, value_ptr(model3));
		RenderScene(&geometry, &shader, &texture);
