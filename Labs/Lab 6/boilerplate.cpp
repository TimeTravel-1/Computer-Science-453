...
void updateCamera(vec3& cameraLoc, vec3& cameraDir, double& phi, double& theta, GLFWwindow* window, int width, int height)
{
	...
	//CHANGE .4F TO SENSITIVITY VARIABLE     
	phi -= x*3.1459265/4.f;
	theta += y*3.14159265/4.f;

	//do not let camera point up or down
	if (theta < .1)
	{
		theta = .1;
	}
...
int main(int argc, char *argv[])
{
  ...
  float aspectRatio = (float)width/ (float)height;
	...
	double phi = 3.14159265, theta = 3.14159265/2.f;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUseProgram(shader.program);
		model = translate(I, vec3(0,2,0))*scale(I, vec3(1000.f, .1f, 1000.f));
		glUniformMatrix4fv(modelUniform, 1, false, value_ptr(model));
		RenderScene(&geometry, &shader);
