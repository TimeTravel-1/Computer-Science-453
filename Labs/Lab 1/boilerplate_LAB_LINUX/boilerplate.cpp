...
bool InitializeTexture(MyTexture* texture, const char* filename, GLuint target = GL_TEXTURE_2D)
{
	...
	unsigned char *data = stbi_load(filename, &texture->width, &texture->height, &n, 0);
	if (data != nullptr)
	{
...
int main(int argc, char *argv[])
{
	...
	/////////////////////////////
	glUseProgram(shader.program);
	GLuint mousePosUniform = glGetUniformLocation(shader.program, "mousePos");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform2f(mousePosUniform, (float) mouseX, (float) (512-mouseY));
